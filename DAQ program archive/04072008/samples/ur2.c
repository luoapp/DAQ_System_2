...
//#define sample_per_frame UDELdaqblocksize
...
int main( int argc, char **argv )
{
  ...
  int retv = 0;
  int sample_per_frame = UDELdaqblocksize;  
  ...
  sample_per_frame = board.ctl.ictl.nchans * ((int) (board.ctl.ictl.clockrate / xwin_fps+1));
  int frame_num, file_num;
  frame_num = (int) length_in_sec *  board.ctl.ictl.nchans * ((float)board.ctl.ictl.clockrate / sample_per_frame) +1 ;
  file_num = (int) frame_num * 2. * (float)sample_per_frame / UDELdaqfilesize  + 1;
  printf( "sample_per_frame = %d cr=%f nch=%d bn=%d fn=%d nrange=%d\n",sample_per_frame, board.ctl.ictl.clockrate,board.ctl.ictl.nchans, frame_num, file_num, board.ctl.ictl.nrange);
  

  /* Malloc two buffers (alignment is optional)
   */
  buffa = daqMallocAligned( buffa,  sample_per_frame*sizeof(unsigned int) );
  if ( !buffa ) { printf( "unable to malloc buffa\n" ); exit( 1 ); }

  buffb = daqMallocAligned( buffb,  sample_per_frame*sizeof(unsigned int) );
  if ( !buffb ) { printf( "unable to malloc buffb\n" ); exit( 1 ); }

  /* Add the buffers to the input dma engineinbuffer
   */
  retv = daqAddInputBuffer( &board, buffa, sample_per_frame );
  if ( retv ) { perror( "add buffa"); exit( 1 ); }

  retv = daqAddInputBuffer( &board, buffb, sample_per_frame );
...
*/
  ...
  for ( ix_f = 0; ix_f <file_num; ix_f++)
    ...
    {
      if ( ix_f == file_num -1 ) bx1 = (int)(frame_num - ix_f * ((float)UDELdaqfilesize / sample_per_frame / 2));
      else bx1 = UDELdaqfilesize / sample_per_frame / 2;
      ...
      { 
		...
		//udelprintstatus( &board.status );
		  for ( ix=0; ix < sample_per_frame ; ix++ ) u16buff[ix] = 0xffff & p[ix];
        //fwrite( u16buff, sample_per_frame, sizeof (unsigned short), pdaqfile);
		if ( flag_hd ) fwrite( p, sample_per_frame, sizeof (unsigned int), pdaqfile);
		//printf( " board.ioverflow = %u\n", board.ioverflow);
		//udelprintstatus( &board.status );

//      printf( " => addr 0x%x avg %.4f s=%d", (unsigned int) p, ravg,  sample_per_frame);

//      p = daqWaitInputBuffer( &board );
//      if ( !p ) break;
//      gscRawtoFloat( rdata, p, sample_per_frame, board.ctl.ictl.vfs );
      
 //     for (ix2=0; ix2<sample_per_frame; ix2++) rdata[ix2]=udelraw2float(p[ix2], board.ctl.ictl.vfs);
      //if (ix_b == 0) memcpy(rdata2, rdata, sample_per_frame*sizeof(float));
      ...
      for ( ix2=0; ix2< update_per_frame; ix2++) rdata_bg[ix2+win_width-update_per_frame]=(int)(udelraw2float( p[((int)ix2*sample_per_frame/board.ctl.ictl.nchans/update_per_frame)*board.ctl.ictl.nchans], board.ctl.ictl.vfs)* win_height /xwin_fs  + win_height/2 ) ;
      ...
      for(ix2=0; ix2<win_width-1; ix2 ++ )
      {
        ...
        //ix3 = (int)(((float)ix2) / win_width * (sample_per_frame-7) / 8) * 8;

        //XDrawPoint(display, win, gc, ix2, rdata_fg[ix2]  );
        /*ix3 = (int)(((float)ix2) / win_width * (sample_per_frame-7) / 8) * 8 +2;
        XDrawPoint(display, win, gc, ix2, (int)(rdata2[ix3] * win_height /0.2  + win_height/2) );
        ix3 = (int)(((float)ix2) / win_width * (sample_per_frame-7) / 8) * 8 +4;
        XDrawPoint(display, win, gc, ix2, (int)(rdata2[ix3] * win_height /0.2  + win_height/2) );
        ix3 = (int)(((float)ix2) / win_width * (sample_per_frame-7) / 8) * 8 +6;
        XDrawPoint(display, win, gc, ix2, (int)(rdata2[ix3] * win_height /0.2  + win_height/2) );*/
...
/*      ravg = avgvalue( rdata, sample_per_frame );
