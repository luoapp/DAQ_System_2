...
//      gscRawtoFloat( rdata, p, NDATA, board.ctl.ictl.vfs );
 ...
 //     for (ix2=0; ix2<NDATA; ix2++) rdata[ix2]=udelraw2float(p[ix2], board.ctl.ictl.vfs);
      ...
      for ( ix2=0; ix2< update_per_frame; ix2++) 
      rdata_bg[ix2+win_width-update_per_frame]=(int)( win_height/2 - udelraw2float( p[((int)ix2*NDATA/board.ctl.ictl.nchans/update_per_frame)*board.ctl.ictl.nchans], board.ctl.ictl.vfs)* win_height /xwin_fs   ) ;
       //printf("%f %f %d\n", udelraw2float( p[((int)ix2*NDATA/board.ctl.ictl.nchans/update_per_frame)*board.ctl.ictl.nchans], board.ctl.ictl.vfs), xwin_fs, win_height);
