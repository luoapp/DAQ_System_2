...
//  XColor blue = puxwin->blue;
  ...
  { 
    if (first_run != 1)
   {
    ...
    pixdata[ix2] = (int)(-(rdata[ix3] - puctl->xwin_offset) * win_height /puctl->xwin_fs  + win_height/2);
    pixdata[ix2+1] = (int)(-(rdata[ix4] - puctl->xwin_offset) * win_height /puctl->xwin_fs  + win_height/2);
