...
//-----------------------------------------------------------------------
  ...
  int bytepersample = sizeof(unsigned int);
  int  ix,  NDATA;
  float *rdata;
  unsigned int * buff;
  XEvent uevent;
  ...
  //NDATA = ( ix2 - 2 * sizeof(UDELdaqheader) - UDELdaqblocksize * bytepersample > 0 ) ? UDELdaqblocksize : (ix2 - 2 * sizeof(UDELdaqheader))/bytepersample; 
  NDATA=128;
