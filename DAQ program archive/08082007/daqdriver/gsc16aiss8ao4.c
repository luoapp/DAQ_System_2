...
#if defined ( DEBUGISR ) | defined ( DAQDRV_DEBUG )
...
int daqDma0StartSeg( DAQDRV_Dev *dev, DAQDRV_DmaSeg *seg )
{
  ...
  DEBUGMSG( "  daqDma0StartSeg seg = 0x%lx\n", (unsigned long) seg );
  ...
  if ( seg && seg->len && ( (seg->controls & SEGIDMASK) == SEGID ) ) 
	...
	iowrite32( seg->len, (dev->plxbase+DMA_CH_0_TRANS_BYTE_CNT) );
      */
      ...
      if ( seg->len/4 <= uval )
	{
	  DEBUGMSG( "    daqDma0StartSeg: buff 0x%x len 0x%x, start\n",
		    uval, seg->len/4 );
	...
	{
	  DEBUGMSG( "    daqDma0StartSeg: buff 0x%x len 0x%x, wait\n",
		       uval, seg->len/4 );
  ...
  else
    {
      ...
      if ( !seg )
	DEBUGMSG( "    daqDma0StartSeg: buff 0x%x, no seg, no start\n", uval );

      else if ( !seg->len )
	DEBUGMSG( "    daqDma0StartSeg: buff 0x%x, len 0, no start\n", uval );

      if ( seg && ((seg->controls & SEGIDMASK) != SEGID) )
	DEBUGMSG( "    daqDma0StartSeg: buff 0x%x, SEGID mismatch, no start\n", uval );
    }

  DEBUGMSG( "    daqDma0StartSeg done, retv %d\n", retv );
...
int daqDma1StartSeg( DAQDRV_Dev *dev, DAQDRV_DmaSeg *seg )
{
  ...
  DEBUGMSG( "  daqDma1StartSeg seg = 0x%lx\n", (unsigned long) seg );
  ...
  if ( seg && seg->len && ( (seg->controls & SEGIDMASK) == SEGID ) )
	...
	{
	  DEBUGMSG( "    daqDma1StartSeg: buff 0x%x len 0x%x, start\n",
		       uval, seg->len/4 );
	...
	{
	  DEBUGMSG( "    daqDma1StartSeg: buff 0x%x, len 0x%x, wait\n",
		       uval, seg->len/4 );

	  // set the threshold (flag goes high-to-low at threshold value)
	  uval = FIFOSIZE - seg->len/4;
	  iowrite32( uval, (dev->iobase+AOUT_BUFF_THRSHLD) );

	  // ones bit, to indicate wait
	  retv = seg->len + 1;
	}
  ...
  else
    {
      ...
      if ( !seg )
	DEBUGMSG( "    daqDma1StartSeg: buff 0x%x, no seg\n", uval );

      else if ( !seg->len )
	DEBUGMSG( "    daqDma1StartSeg: buff 0x%x, len 0\n", uval );

      if ( seg && (seg->controls & SEGIDMASK) != SEGID )
	DEBUGMSG( "    daqDma1StartSeg: buff 0x%x, SEGID mismatch\n",
		  uval );

    }

  DEBUGMSG( "    daqDma1StartSeg done, retv %d\n", retv );
...
int daqDma1Start(  DAQDRV_Dev *dev )
{
  ...
  // start the DMA, includes load, start as needed
  ...
  */
  ...
  if ( BITMATCH( plxcsr , (IRQ_LOCAL_ENABLE | IRQ_LOCAL_ACTIVE) ) )
      ...
      else if ( BITMATCH( daqicr, (ICR_OUTPUT_HIGH_TO_LOW | ICR_OUTPUT_HIGH_TO_LOW_SELECT) ) )
	{
	  ...
	  if ( dev->dma1.enabled )
	    {
	      /* Clear the Threshold First(downwards).
  ...
  if ( BITMATCH( plxcsr , (IRQ_DMA_0_ENABLE | IRQ_DMA_0_ACTIVE) ) )
      ...
      if ( !(dmacsr & DMA0_DONE) )
	{
      ...
      else if ( !dma->enabled || !seg )
	{
	  DAQDRVMSG( "  isr, dma0 done, dma0 not enabled or no seg!!\n" );
      ...
      else if ( (n=daqDma0StartSeg( dev, seg->next )) )
	{
	  if ( (seg->controls & SEGINTBIT) )
	    {
	      DEBUGISRMSG( "  isr, dma0 done, do next, previous SEGINTBIT set, signalling POLL_IN\n" );
	  ...
	  else if ( n & 1 )
	    {
	      ...
	      DEBUGISRMSG( "  isr, dma0 done, waiting, icr 0x%x thr 0x%x buf 0x%x\n",
			   daqicr, ithresh, ibuff );
	    }
	  else
	    {
	      DEBUGISRMSG( "  isr, dma0 done, started\n" );
	    }
	}
  ...
  if ( BITMATCH( plxcsr , (IRQ_DMA_1_ENABLE | IRQ_DMA_1_ACTIVE) ) )
    {
      ...
      if ( !(dmacsr & DMA1_DONE) )
	{
	  DAQDRVMSG( "  isr, dma1 done,no DMA1_DONE bit!!\n" );
	...
	{
	  DAQDRVMSG( "  isr, dma1 done, dma1 not enabled or no seg!!\n" );
      ...
      else if ( (n=daqDma1StartSeg( dev, seg->next )) )
	{
	  if ( (seg->controls & SEGINTBIT) )
	    {
	      DEBUGISRMSG( "  isr, dma1 done, do next,  previous SEGINTBIT set, signalling POLL_OUT\n" );
	    ...
	    {
	      DEBUGISRMSG( "  isr, dma1 done, waiting\n" );
	    }
	  else
	    {
	      DEBUGISRMSG( "  isr, dma1 done, started\n" );
	    }
	}
      ...
      else
	{
	  ...
	  // We're not signalling, fifo should do that when it gets to zero
	  //  DEBUGMSG( "  isr, adding signal POLL_OUT\n" );
	  //  siglist[nsigs] = POLL_OUT;
	  //  nsigs++;
	}
    }
