...
int main( )
{
  ...
  board.ctl.ictl.vfs = 5;

  buffa = daqMallocAligned( buffa,  NDATA*sizeof(unsigned int) );

  fread(buffa, NDATA, sizeof(unsigned int), pdaqfile);
  gscRawtoFloat( rdata, buffa, NDATA, 5 );
  ravg = avgvalue( rdata, NDATA );
  printf( " => avg %.4f\n",  ravg );
  printRawFlt( stdout, buffa, NDATA, board.ctl.ictl.nchans,	board.ctl.ictl.vfs        );
  exit(0);
  ...
  while( n < 10 )
    {
      ...
      ravg = avgvalue( rdata, NDATA );
		//printRawFlt( pdaqfile, p, 1, board.ctl.ictl.nchans,
	   //   board.ctl.ictl.vfs );
		//fprintf( pdaqfile, "%f\n", rdata[0] );
