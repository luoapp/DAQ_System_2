...
void daqdrv_testsglist( struct scatterlist *sg, int nsg )
{
  ...
  DEBUGMSG( "testsglist, sg=0x%x nsg=%d\n", (unsigned int) sg, nsg );
...
void daqdrv_dma_release_userpages_( DAQDRV_Dev *daqdev, DAQDRV_DmaUser *dmauser, int channel )
{
  ...
  DEBUGMSG( "**dma_release_userpages_ dmauser 0x%x channel %d useraddress=0x%x, bytes=%d\n",
	    (unsigned int) dmauser, channel, dmauser->uaddr, dmauser->usize );

  if ( dmauser->seg )
    {
      DEBUGMSG( "  dma_release_userpages_ kfree seg 0x%x\n", (unsigned int) dmauser->seg );
  ...
  if ( dmauser->sg )
    {
      DEBUGMSG( "  dma_release_userpages_ unmap and kfree sg 0x%x\n", (unsigned int) dmauser->sg );
  ...
  if ( dmauser->pages )
    {
      DEBUGMSG( "  dma_release_userpages_ dirty and release %d pages 0x%x\n",
		dmauser->npages, (unsigned int) dmauser->pages );
  ...
  while( dma->userhead )
    {
      ...
      // release its pages
      DEBUGMSG( "  dma_release_ channel %d dmauser 0x%x\n", channel, (unsigned int) dmauser );
...
int daqdrv_dma_add_( DAQDRV_Dev *daqdev, unsigned int useraddress, int lenbytes, int channel )
{
  int n = 0;
  int n0,nl;
  int m;
  ...
  DEBUGMSG( "**dma_add channel %d useraddress=0x%x, bytes=%d\n",
  ...
  dmauser->npages = get_user_pages( current, current->mm, useraddress, lenpages, 1, 0, dmauser->pages, NULL );

  up_read( &current->mm->mmap_sem );

  if ( !dmauser->npages )
    {
      DEBUGMSG( "dma_add get user pages 0x%x failed\n", (unsigned int) useraddress );
      daqdrv_dma_release_userpages_( daqdev, dmauser, channel );
      return( -1 );
    }
  ...
  for( m = 0; m < dmauser->nseg; m++ )
    {
      DEBUGMSG( "  dma_add seg 0x%x  addr 0x%x len %d next 0x%x\n",
		(unsigned int) &dmauser->seg[m],
		dmauser->seg[m].addr,
		dmauser->seg[m].len,
		(unsigned int) dmauser->seg[m].next );
    }
  DEBUGMSG( "  dma_add seghead 0x%x segtail 0x%x\n",
	    (unsigned int) dma->seghead, (unsigned int) dma->segtail );
#endif

  DEBUGMSG( "==dma_add channel %d  useraddress=0x%x, bytes=%d nseg=%d done\n",
...
int daqdrv_open( struct inode *inode, struct file *filp )
{
  ...
  */
  ...
  /* now we should be connected, we can re-enable interrutps
  */
...
int daqdrv_ioctl( struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg )
{
  ...
  if ( (ndev < MAXDAQDEV) && devlist[ndev].pcidev )
	...
	case DAQDRV_IOC_SET_USERDATA:
	  ...
	  if ( (fiforeq.ndata <= 0) || !access_ok(VERIFY_WRITE, fiforeq.data, (fiforeq.ndata*4) ) )
	    {
	      DEBUGMSG( "ioctl, add_input_buffer invalid buffer 0x%x %d words\n",
		      (unsigned int) fiforeq.data, fiforeq.ndata );
	      return -EFAULT;
	    }

	  DEBUGMSG( "ioctl, add_int_buffer dma0: 0x%x %d words\n",
		    (unsigned int) fiforeq.data, fiforeq.ndata );

	  n = daqdrv_dma_add_( &devlist[ndev], 
			       (unsigned int) fiforeq.data,
			       fiforeq.ndata*sizeof(int), 
			       0 );
	  ...
	  if ( (fiforeq.ndata <= 0) || !access_ok(VERIFY_READ, fiforeq.data, (fiforeq.ndata*4) ) )
	    {
	      DEBUGMSG( "ioctl, add_output_buffer invalid buffer 0x%x %d words\n",
		      (unsigned int) fiforeq.data, fiforeq.ndata );
	      return -EFAULT;
	    }

	  DEBUGMSG( "ioctl, add_output_buffer dma1: 0x%x %d words\n",
		    (unsigned int) fiforeq.data, fiforeq.ndata );
	  ...
	  if (!access_ok(VERIFY_WRITE, fiforeq.data, (fiforeq.ndata*4) ) )
	    {
	      DEBUGMSG( "ioctl, fiforead VERIFY WRITE failed 0x%x %d\n",
		      (unsigned int) fiforeq.data, fiforeq.ndata );
	      return -EFAULT;
	    }
	  ...
	  if (!access_ok(VERIFY_READ, fiforeq.data, (fiforeq.ndata*4) ) )
	    {
	      DEBUGMSG( "ioctl, fifowrite VERIFY WRITE failed 0x%x %d\n",
		      (unsigned int) fiforeq.data, fiforeq.ndata );
	      return -EFAULT;
	    }
...
static int probe( struct pci_dev *dev, const struct pci_device_id *id)
{
  ...
  */
  ...
  printk( KERN_NOTICE "daqdrv: irq = 0x%x (%d)\n", u8temp, u8temp );
...
static int __init daqdrv_init(void)
{
  ...
  printk( KERN_NOTICE "daqdrv (data acquisition card driver)"
	  " Copyright 2006, Mitchell C. Nelson, Ph.D., mcnelson@mindspring.com, Sensor Realtime, LLC\n" );
